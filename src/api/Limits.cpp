  ...
  switch(api)
  {
    ...
    case ApiType::OpenWeatherMap:
         switch (planOwm)
         {
             ...
             case PlanOwm::Expert:
                  /* Limit for OpenWeatherMap is 100000 calls per minute on the Expert plan. */
                  return Limit(100000, std::chrono::minutes(1));
    ...
    case ApiType::Weatherbit:
         switch (planWb)
         {
             ...
             case PlanWeatherbit::Standard:
                  /* Limit for Weatherbit is 25000 calls per day on the Standard plan,
                     which is ca. 1041.67 requests per hour. */
                  return Limit(25000, std::chrono::hours(24));
             case PlanWeatherbit::Plus:
                  /* Limit for Weatherbit is 250000 calls per day on the Plus plan,
                     which is ca. 10416.67 requests per hour. */
                 return Limit(250000, std::chrono::hours(24));
             case PlanWeatherbit::Business:
                  /* Limit for Weatherbit is 2000000 calls per day on the Business plan,
                     which is ca. 83333.33 requests per hour. */
                 return Limit(2000000, std::chrono::hours(24));
