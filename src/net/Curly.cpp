...
bool Curly::addPostField(const std::string& name, const std::string& value)
{
  /*Perform some checks before adding the post field:
    No empty names, avoid conflict with file field names (if we still had files),
    and do not set it, if we already have a plain post body. */
...
{
  //avoid conflicts with post fields (and files, if the code would still support it)
